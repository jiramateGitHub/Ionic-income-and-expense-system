...
export class MWalletService {
  ...
  get_wallet_balance() {
    this.serviceCollection = this.afs.collection<MWallet>('M_wallet', ref => ref.where('username', '==', this.SessionService.get_session_username()).where('wallet_name', '==', this.SessionService.get_session_wallet()));
    this.service = this.serviceCollection.snapshotChanges().pipe(
      map(actions => {
        return actions.map(a => {
          const data = a.payload.doc.data();
